...
import { appendICLog } from "./utils/appendICLog";
import { checkCallword } from "./utils/checkCallword";
...
export const set_side = async ({
  ...
  if (showSpeedLines === true) {
    ...
    court.src = await tryUrls(backgroundFolder + bg);
  }

  if (showDesk === true && desk) {
    const deskFilename = (await fileExists(backgroundFolder + desk.ao2))
      ? desk.ao2
      : desk.ao1;
    bench.src = backgroundFolder + deskFilename;
...
export const handle_ic_speaking = async (playerChatMsg: ChatMsg) => {
  ...
  if (shout) {
    ...
    shoutaudio.play();
    shoutTimer = client.resources[shout].duration;
  } else {
    shoutTimer = 0;
  }
...
const chat_tick = async () => {
  ...
  const isShoutAndPreanimOver = tickTimer >= shoutTimer + chatmsg.preanimdelay;
